...
// copia/incolla l’intero file così com’è.
...
let socketReady    = false;                // true dopo RECONCILE OK
...
async function refreshToken (delay = 0) {
  ...
  try {
    ...
    setTimeout(
      refreshToken,
      (expires - 60 + (Math.random()*10 - 5))*1000          // rinnovo ~1 min prima
    );
  } catch (err) {
...
function openSocket () {
  ...
  ws.on('open', () => {
    ...
    sendWS({
      ...
      accessToken                               // (opz.) – non indispensabile qui
    })
    .then(() => {
    ...
    .then(() => {
      console.log('✓ Account-auth OK – sending RECONCILE');
      return sendWS({
        payloadType        : 70,              // ProtoOAReconcileReq
        ctidTraderAccountId: Number(CTRADER_ACCOUNT_ID),
        lastOrderUpdateTimestamp : 0,
        lastPositionUpdateTimestamp : 0,
        lastDealUpdateTimestamp    : 0
      });
    })
    .then(() => {
      socketReady = true;
      console.log('✓ Reconcile OK — socket pronto');
    })
    .catch(err => {
...
app.post('/order', async (req, res) => {
  ...
  const {
    symbolId,       // INT   (obbligatorio)
    side,           // "BUY" / "SELL"
    volume,         // in cent units (es. 100_000 = 1 lot)
    type = 'MARKET',
    limitPrice,
    stopPrice,
    tp, sl,
...
/* BOOT                                                               */
/* ------------------------------------------------------------------ */
await refreshToken();           // ottiene HTTP token all’avvio
openSocket();                   // handshake WS + auto-reconnect
